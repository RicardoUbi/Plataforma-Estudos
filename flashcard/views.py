...
def relatorio(request, id):
    ...
    for categoria in categorias:
        ...
        dados_categoria[categoria.nome] = {'acertos': acertos_categoria, 'erros': erros_categoria}

    melhores_materias = []
    piores_materias = []

    if len(dados_categoria) > 0:
        # Ordenar as categorias pelo número de acertos em ordem decrescente
        melhores_materias = sorted(dados_categoria.items(), key=lambda item: item[1]['acertos'], reverse=True)
        print("Melhores matérias:", melhores_materias)

        # Ordenar as categorias pelo número de erros em ordem decrescente
        piores_materias = sorted(dados_categoria.items(), key=lambda item: item[1]['erros'], reverse=True)
        print("Piores matérias:", piores_materias)
